    ...
    function getInitializeResultArray(Vote[] memory _votes, uint[] memory _choiceIds) private pure returns(uint[4][4] memory) {
        ...
        for(uint i = 0; i < _choiceIds.length+1; i++) {
            for(uint j = 0; j < _votes[i].choiceIds.length; j++) {
                if ( _votes[i].choiceIds[j] == _choiceIds[0]) result[0][j]++;
                else if ( _votes[i].choiceIds[j] == _choiceIds[1]) result[1][j]++;
                else if ( _votes[i].choiceIds[j] == _choiceIds[2]) result[2][j]++;
                else if ( _votes[i].choiceIds[j] == _choiceIds[3]) result[3][j]++;
            }
        }
        return result;
    }
